...
getwd()
setwd("~/Desktop/Current Research/Emmas Research/Squamates")
...
setwd("~/Desktop/Current Research")
getwd()
...
#take the NA out
Dielsqua1<-
  Dielsqua %>% drop_na(Diel)
View(Dielsqua1)
#501

#DIURNAL=1, NOCTURNAL=0, ARRYTHMIC=1, CRESPUCULAR=0
Dielsqua1['Diel'][Dielsqua1['Diel'] == 'DIU'] <- '1'
View(Dielsqua1)

Dielsqua1['Diel'][Dielsqua1['Diel'] == 'NOC'] <- '0'
View(Dielsqua1)

Dielsqua1['Diel'][Dielsqua1['Diel'] == 'ARR'] <- '1'
View(Dielsqua1)


Dielsqua1['Diel'][Dielsqua1['Diel'] == 'CRE'] <- '0'
View(Dielsqua1)
...
Dielsqua1$species == squa_tree$tip.label

squa_complete1$species == squa_tree$tip.label

# Compare taxa in data and tree
library(geiger)
#squa_complete1 doesnt not have Diel
name.check(squa_tree, Dielsqua1)

# Match species in squa_complete with squa_tree phylogeny tips
View(squa_tree)
View(Dielsqua1)
squa_match1 <- Dielsqua1[match(squa_tree$tip.label,
                               Dielsqua1$species), ]
View(squa_match1)
squa_match1$species == squa_tree$tip.label

# Make df with matched squamate species and tree tip labels 
td_squa1 <- make.treedata(squa_tree, squa_match1)
View(td_squa1$dat)
#566 without Diel
#453 with Diel
...
td_squa1 <- filter(td_squa1, !is.na(precipmm), !is.na(tempC), !is.na(Tb))

ME_squa_replace1 <- td_squa1$dat$Tb.err
ME_squa_replace1 <- ME_squa_replace1 %>% replace_na(0.45)
View(ME_squa_replace1)
View(data.frame(ME_squa_replace1))

View(data.frame(td_squa1$dat))  #422
...
diel_asr <- ace(x=setNames(factor(td_squa1$dat$Diel+1), tree_squa_diel$tip.label), phy=tree_squa_diel, type="discrete")
...
tiplabels(pch=21, bg=td_squa1$dat$Diel+1, col=td_squa1$dat$Diel+1, cex=0.5)
...
sp_values_df_endo <- readRDS("./Mammals/sp_values_df_endo.RDS")
View(sp_values_df_endo)
...
setwd("~/Desktop/Current Research/Endo&EctoFile/Mammals")
# Mammal thermo data
endo_dat <- readRDS("~/Desktop/Current Research/Endo&EctoFile/Mammals/Tbdat.rds")
View(endo_dat)
...
# Endotherm tree 
endo_tree <- endo_dat$phy
View(endo_tree)
endo_tree$tip.label

max(branching.times(endo_tree))

#??ANOTHER TREE USED TO PLOT?
plot(td_endo$phy, show.tip.label = FALSE)



# Remove "_" in between genus and species for tip labels
endo_tree$tip.label <- gsub("_", " ", endo_tree$tip.label)
endo_tree$tip.label
class(endo_tree)

endo_complete$species == endo_tree$tip.label

# Compare taxa in data and tree
name.check(endo_tree, endo_complete)

#WITHOUT DIEL
# Match species in endo_complete with endo_tree phylogeny tips
#endo_match <- endo_complete[match(endo_tree$tip.label,
           #                       endo_complete$species), ]
#View(endo_match)
#endo_match$species == endo_tree$tip.label

#nrow(endo_match) #857


#WITH DIEL. USE THIS
#endo_matchD <- Dielendo1[match(endo_tree$tip.label,
#                               Dielendo1$species), ]
left_join(endo_complete, Dielendo1, by="species")
View(endo_matchD)
endo_matchD$species == endo_tree$tip.label
...
# Make df with matched endo species and tree tip labels 
td_endo <- make.treedata(endo_tree, endo_matchD)
td_endo$phy$tip.label
td_endo[["Tb"]]
td_endo$dat
nrow(td_endo$dat) #448

# Save as RDS 
saveRDS(td_endo, file = "td_endo.RDS")
td_endo <- readRDS("td_endo.RDS")
class(td_endo)
...
ME_endo_replace <- ME_endo_replace %>% replace_na(0.144463)
View(ME_endo_replace)
View(data.frame(ME_endo_replace))


# Filter out NA's within Tb for species 
td_endo <- dplyr::filter(td_endo, !is.na(Tb))
View(td_endo$phy$tip.label)
View(td_endo$dat)
...
diel_asr <- ace(x=setNames(factor( td_endo$dat$Diel+1), endo_tree_diel$tip.label), phy=endo_tree_diel, type="discrete")
...
tiplabels(pch=21, bg= td_endo$dat$Diel+1, col= td_endo$dat$Diel+1, cex=0.5)
...
model4a <- slouch.fit(phy = endo_tree_diel, 
                      ...
                      fixed.fact = td_endo[['Diel']])
model4a
summary(model4a)
######

#####WITH DIEL DELETE (THIS IS FOR ECTORTHERMS)
tree_squa_diel <- td_squa1$phy
diel_asr <- ace(x=setNames(factor(td_squa1$dat$Diel+1), tree_squa_diel$tip.label), phy=tree_squa_diel, type="discrete")
tree_squa_diel$node.label <- unname(apply(diel_asr$lik.anc, 1, function(x) which(x==max(x))))-1
...
temp_squa_reg <- ggplot(td_squa1$dat,
  ...
  geom_abline(slope=model1$beta_primary$coefficients_bias_corr[3,1],
              intercept = model1$beta_primary$coefficients_bias_corr[1,1],
              size=1, colour="lightgreen") +
  geom_abline(slope=model1$beta_evolutionary$coefficients_bias_corr[3,1],
              intercept = model1$beta_evolutionary$coefficients_bias_corr[1,1],
              linetype="dashed", size=1, colour="lightblue") +
  scale_x_continuous(limit=c(0,40), breaks=c(5,10,15,20,25,30,35,40),
                     labels=c("5","10","15","20","25","30","35","40"),
                     expand = c(0,0)) +
  scale_y_continuous(limit=c(0,60), breaks=c(5,10,15,20,25,30,35,40,
                                             45,50,55,60),
...
temp_endo_reg <- ggplot(td_endo$dat,
  ...
  geom_abline(slope=model4$beta_evolutionary$coefficients_bias_corr[3,1],
              ...
              linetype="dashed", size=1, colour="lightblue") +
  scale_x_continuous(limit=c(-25,40), breaks=c(-25,-20,-15,-10,-5,0,5,10,15,20,25,30,35,40),
                     labels=c("-25","-20","-15","-10","-5","0","5","10","15","20","25","30","35","40"),
                     expand = c(0,0)) +
  scale_y_continuous(limit=c(25,45), breaks=c(25,30,35,40,
                                              45),
                     labels = c("25","30","35",
                                "40","45"),
                     expand = c(0,0)) +
...
precip_endo_reg <- ggplot(td_endo$dat,
  ...
  theme_minimal() +
  theme(plot.title = element_text(hjust=0.5, size=20, face="plain")) +
  geom_abline(slope=model4$beta_primary$coefficients_bias_corr[2,1],
              intercept = model4$beta_primary$coefficients_bias_corr[1,1],
              size=1, colour="lightgreen") +
  geom_abline(slope=model4$beta_evolutionary$coefficients_bias_corr[2,1],
              intercept = model4$beta_evolutionary$coefficients_bias_corr[1,1],
              linetype="dashed", size=1, colour="lightblue") +
...
squa_phy <- td_squa$phy
squa_Tb <- c(td_squa$phy$tip.label, td_squa$dat$Tb) 
species <- squa_Tb[1:188]
View(species)
Tb <- squa_Tb[189:376]
View(Tb)
...
ME_squa <- cbind(species, ME_squa_replace)
ME_squa

ME_squa <- as.data.frame(ME_squa)
ME_squa <- setNames(as.numeric(ME_squa$ME), ME_squa$species)
ME_squa

fitBM_squa <- fitContinuous(squa_phy, squa_Tb, model = "BM", SE=ME_squa)
fitBM_squa
summary(fitBM_squa)

# fitContinuous for temp and precip in Squamates
# Enviro Temp for Squamates
temp <- td_squa$dat$tempC
